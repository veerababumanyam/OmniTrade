 ...
 */
const getVariantIcon = (variant: AlertVariant): IconName => {
 ...
 * Get variant class
 */
const getVariantClass = (variant: AlertVariant): string => {
...
const getSizeClass = (size: 'sm' | 'md' | 'lg'): string => {
  const classMap: Record<'sm' | 'md' | 'lg', string> = {
    sm: styles.sizeSm,
    md: styles.sizeMd,
    lg: styles.sizeLg,
  };
...
export const Alert: React.FC<AlertProps> = ({
  ...
  const handleDismiss = useCallback(
    (autoClosed = false) => {
      ...
      // Emit signal
      const signalPayload: AlertDismissSignalPayload = {
        alertId: id || alertId,
        variant,
      ...
      signalBus.publish('ui:alert:dismiss', signalPayload);
    ...
    [id, alertId, variant, title, stackIndex, onDismiss]
  );
  ...
  useEffect(() => {
    if (autoClose > 0 && progressRef.current) {
      progressRef.current.style.animationDuration = `${autoClose}ms`;
    }
  }, [autoClose]);
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      if (dismissible && (event.key === 'Escape' || event.key === 'Dismiss')) {
        handleDismiss();
      }
    },
  ...
  const renderAction = (action: AlertAction, index: number) => {
    ...
    return (
      <button
        key={index}
        type="button"
        className={cn(styles.alertActionButton, variantClass)}
        onClick={action.onClick}
        disabled={action.disabled || action.loading}
        aria-busy={action.loading}
      >
        {action.loading && (
          <Icon name="loading" className={styles.alertIconSpinning} />
        )}
        {action.label}
      </button>
    );
  ...
  return (
    <div
      ...
      className={cn(
        ...
        getSizeClass(size),
        glass && styles.alertGlass,
        dismissible && styles.alertDismissible,
        animated && !isExiting && styles.alertAnimatedIn,
        animated && isExiting && styles.alertAnimatedOut,
        stackIndex !== undefined && styles.alertStackedItem,
        className
      )}
    ...
    >
      ...
      {showIcon && displayIcon && (
        <div
          className={cn(
            styles.alertIcon,
            displayIcon === 'loading' && styles.alertIconSpinning
          )}
        >

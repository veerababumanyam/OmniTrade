...
2. **Intelligence Service (Go/Genkit)**: Hosts the Genkit flows and agents. Uses the `omnitrade_readonly` database role, and relies on Redis 8.x for Rate Limiting and Semantic Caching.
...
### 2.4 Redis Configuration (Caching & Rate Limiting)
Redis 8.x is used heavily to prevent expensive database or LLM queries for repeated actions:
- **Rate Limiting**: Protects backend endpoints (using Token Bucket or Sliding Window algorithms).

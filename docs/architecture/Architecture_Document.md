...
services:
  ...
  # Provides: load balancing, fallback/retry, cost tracking,
  # virtual keys, MCP gateway, and A2A protocol support.
  ...
  litellm:
    ...
    environment:
      ...
      LITELLM_DATABASE_URL: "postgresql://omnitrade:${DB_PASSWORD}@db:5432/omnitrade"
    ...
    labels:
      ...
      - "traefik.http.middlewares.litellm-strip.stripprefix.prefixes=/llm"
...
## 10. Universal LLM Provider Abstraction — LiteLLM AI Gateway

> **LiteLLM** (MIT License) serves as OmniTrade's **Universal LLM Gateway**, providing a single OpenAI-compatible API endpoint that routes to 100+ LLM providers. This replaces the need for a custom Go provider factory — all LLM calls go through `http://litellm:4000/chat/completions`.
...
graph LR
    ...
    ROUTER --> LITELLM["LiteLLM AI Gateway<br/>━━━━━━━━━━━<br/>:4000 · MIT License<br/>OpenAI-compatible API"]
    LITELLM --> LB["Load Balancer<br/>━━━━━━━━━━━<br/>Latency-based routing<br/>Fallback cascades"]
    LB --> CLOUD["Cloud Providers<br/>━━━━━━━━━━━<br/>OpenAI · Anthropic<br/>Gemini · DeepSeek · Grok"]
    LB --> LOCAL["Local Providers<br/>━━━━━━━━━━━<br/>Ollama · vLLM<br/>Custom Endpoints"]
    LITELLM --> CACHE["Redis Cache<br/>━━━━━━━━━━━<br/>Response caching<br/>TTL: 5min"]
    LITELLM --> PG["PostgreSQL<br/>━━━━━━━━━━━<br/>Virtual keys<br/>Cost tracking"]
```

### 10.3 Why LiteLLM (Not Kong)

| Criterion | Kong Gateway | LiteLLM |
|:----------|:------------|:--------|
| **License** | Apache 2.0 OSS, AI plugins **Enterprise-only** | **MIT** ✅ |
| **Purpose** | General API gateway | **Purpose-built LLM proxy** |
| **Providers** | ~20 (enterprise) | **100+** out of the box |
| **Load Balancing** | Enterprise-only | ✅ Built-in (latency, round-robin, least-busy) |
| **Fallback/Retry** | Enterprise-only | ✅ Built-in per-model fallback chains |
| **Cost Tracking** | Enterprise-only | ✅ Per-key/per-model token & cost tracking |
| **MCP Gateway** | Enterprise-only | ✅ Built-in MCP + A2A protocol support |
| **Runs with Traefik?** | ❌ Would replace it | ✅ Complementary (LLM proxy only) |

### 10.4 Key Design Decisions

- **LiteLLM Proxy**: All LLM calls from Go backend route through `http://litellm:4000` — a single OpenAI-compatible endpoint handles all 100+ providers
- **No Custom Provider Code**: LiteLLM handles provider-specific API translation, eliminating the need for Go adapter classes per provider
...
- **Fallback Cascades**: Configured in `litellm_config.yaml` — e.g., Claude 500 → GPT-5-mini → local DeepSeek
- **Cost Tracking**: LiteLLM tracks token usage and cost per virtual key, per model, with budget limits
...
| `/llm/*` | LiteLLM AI Gateway (:4000) | Universal LLM proxy (strip prefix) |
...
| **LLM Gateway** | LiteLLM | Universal LLM proxy (100+ providers) | MIT | latest |
...
| `LITELLM_URL` | `http://litellm:4000` | LiteLLM AI Gateway endpoint |
| `LITELLM_MASTER_KEY` | `sk-omnitrade-xxx` | LiteLLM admin key (must start with sk-) |
| `LITELLM_SALT_KEY` | `sk-omnitrade-salt-xxx` | LiteLLM encryption salt (set once, never change) |
